...
Phi::Phi(parmaters *csa) {
    phivalue =
        this->PreSpace(csa); // compute the phi and estimate the space for compression encoding when compressing phi
...
i64 Phi::RightBoundary(i64 r, i64 L, i64 R) { return phi->RightBoundary(r, L, R); }

i64 Phi::LeftBoundary(i64 l, i64 L, i64 R) { return phi->LeftBoundary(l, L, R); }
...
integer Phi::load(loadkit &s) {
    ...
    for (int i = 0; i < 256; i++) doublec[i] = new i64[256];
    for (int i = 0; i < 256; i++) {
...
i64 *Phi::PreSpace(parmaters *csa) {
    ...
    for (i64 i = 0; i < csa->alphabetsize + 1; i++) temp[i] = csa->start[i];
    ...
    for (i = 0; i < x + 1; i++) {
        for (j = i * a; j < (i + 1) * a && j < n; j++) {
            if (j % b == 0) {
                len_g = 0;
                pre = phivalue[j];
                if ((j / b + 1) * b < n) len_bv = phivalue[(j / b + 1) * b] - phivalue[j];
                continue;
            }
            gap = phivalue[j] - pre;
            if (gap < 0) {
...
void Phi::randchar(uchar *T) {
    ...
    this->mini = 200;
    this->doublec = new i64 *[256];
    for (i64 i = 0; i < 256; i++) doublec[i] = new i64[256];
    for (i64 i = 0; i < 256; i++)
        for (i64 j = 0; j < 256; j++) doublec[i][j] = -1;
