...
GEN_Phi::GEN_Phi(parmaters *csa, i64 *phivalue, double multi) {
    ...
    this->SamplingAndCoding(csa); // compressing phi
...
void GEN_Phi::InitionalTables_Gamma() {
    ...
    for (integer i = 0; i < D; i++)
        for (integer j = (1 << i); j < (2 << i); j++) R[j] = D - 1 - i;
    R[0] = D;
...
void GEN_Phi::Testforgetphivalue() {
    ...
    for (i64 i = 0; i < n; i++) {
        if (this->GetValue(i) != phivalue[i]) {
            cout << "i is: " << i << (int)this->method->GetValue(i / b) << " " << this->GetValue(i) << " "
                 << phivalue[i] << endl;
            c--;
            if (!c) exit(0);
        }
...
i64 GEN_Phi::DecodeDeltaBlock(i64 position, i64 base, i64 num) {
    ...
    while (i < num) {
        ...
        if (value % 2 == 0) {
            if (i + value / 2 >= num) return (base + num - i) % n;
            base = (base + value / 2) % n;
            i = i + value / 2;
        } else {
...
void GEN_Phi::SamplingAndCoding(parmaters *csa) {
    ...
    for (i = 0; i < lenofsequence; i++) sequence[i] = 0;
    ...
    for (i64 i = 0; i < n; i++) {
        ...
        if (i % b == 0) {
            ...
            if (m > 3 || m < 0) cout << "WRONG " << method << " " << i / b << endl;
            continue;
        }
        gap = phivalue[i] - pre;
        if (gap < 0) gap = n + gap;
...
i64 GEN_Phi::GetMemorySize() {
    return samples->GetMemorySize() + this->offsects->GetMemorySize() + this->lenofsequence * 4 +
           this->lenofsuperoffset * 8 + method->GetMemorySize();
}
